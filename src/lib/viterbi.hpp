...
double emission_func(const double* const& emission, const double& di, const int& state) {
...
*/
int* viterbi(const double* const& observations, const std::size_t& num_observation, const double* const& initial, const double* const& transition, const double* const& emission) {
    ...
    double* transition_log10 = new double[3*3]();
    ...
    for (std::size_t t = 1; t < num_observation; ++t) {
        for (std::size_t i = 0; i < 3; ++i) {   // current state
            ...
            for (std::size_t j = 0; j < 3; ++j) {   // previous state
                double temp = viterbi[j * num_observation + t - 1] + transition_log10[j * 3 + i];
                if (temp > max) {
                    max = temp;
